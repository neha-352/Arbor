-- create database bus management
...
CREATE TABLE BusDetails
 ...
 BusType VARCHAR(20)) -- e.g., AC, Non-AC, Sleeper, etc. ;
  ...
  INSERT INTO BusDetails (BusID, BusNumber, OperatorName, SourceCity, DestinationCity, DepartureTime, ArrivalTime, Fare, BusType) VALUES
 ...
 --  Display BusID, BusNumber, and Fare for all buses whose fare is greater than ₹600.
 Select BusID,BusNumber,Fare
 from Busdetails
 where fare > 600 ;
 
 -- Return the details of buses that travel from Pune to Mumbai.
 select * from busdetails
 where destinationcity = "Mumbai" and sourcecity = "Pune"; 
 
 -- Show BusNumber and DepartureTime for buses departing after 20:00 (8 p.m.)
 select BusNumber,DepartureTime
 from Busdetails
 where departuretime >'08:00:00';
 
 -- Fetch all columns for buses whose BusType is either AC or Sleeper.
 select * from busdetails
 where BusType = "AC" or "sleeper";
 
 -- Retrieve BusID, OperatorName, and Fare for buses with fares between ₹350 and ₹600 (inclusive).
  select BusID,OperatorName,fare
 from busdetails
 where fare between 350 and 600;
 
 -- List the BusNumbers containing the substring MH12 anywhere in the plate.
 select BusNumber
 from busdetails
 where busnumber  like "MH12%";
 
 --  Show every bus that arrives before 09:00
 Select operatorname
 from busdetails
 where arrivaltime < "09:00:00";
 
 -- Get BusID, SourceCity, DestinationCity, and Fare for buses where the source city is not Pune
 select BusID,sourcecity,destinationcity,fare
 from busdetails
 where sourcecity != "Pune";
 
 --  Display the five cheapest buses (all columns), ordered by Fare ascending and limited to 5 rows. 
 select * from busdetails
 order by fare asc
 limit 5;
 
 -- Return BusNumber and OperatorName for buses whose destination city is in (Goa, Nagpur, Hyderabad).
 select busnumber,operatorname
 from busdetails
 where destinationcity in ("Goa" , "Nagpur" , "Hyderabad");
 
 -- List buses whose DepartureTime is exactly 06:00:00. 
 select operatorname
 from busdetails
 where departuretime = '06:00:00';
 
 -- Fetch details of buses whose BusType is Non-AC and whose fare is less than ₹400.
 select operatorname
 from busdetails
 where bustype="non-ac" and fare<400;
 
 -- Show BusID, BusNumber, Fare for buses travelling from Mumbai and having Fare ≥ ₹500
 select busid,busnumber,fare
 from busdetails
 where sourcecity ="mumbai" and fare > 500;
 
 -- Retrieve all buses ordered by DepartureTime descending (latest departures first).
 select *
 from busdetails
 order by departuretime desc;
 
 -- Select BusNumber, SourceCity, DestinationCity where the destination city name ends with the letter e
 select busnumber,sourcecity,destinationcity
 from busdetails
 where destinationcity like "%e";
 
 -- List buses operated by MSRTC excluding those that start from Mumbai.
 select *
 from busdetails
 where  operatorname = "msrtc" and sourcecity ="mumbai";
 
 -- Get the first three rows (any order) from BusDetails to check the table structure.
 select *
 from busdetails
 order by busid desc
 limit 3;
 
 -- Display BusID, BusType for buses whose BusType is Sleeper and arrival is after 06:00. 
 select busid,bustype
 from busdetails
 where bustype = "sleeper" and arrivaltime > "06:00:00";
 
 -- Return all buses where Fare is not equal to ₹450
 select *
 from busdetails
 where fare != 450;