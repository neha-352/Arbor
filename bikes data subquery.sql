...
select brand from bikes where price > (select avg(price) from bikes where type='mountain');
...
select brand from bikes where brand in ( select brand from bikes group by brand having sum(stock) > 3);
...
select * from bikes where price > (select max(price) from bikes where type='hybrid');

-- Q10: Find bikes whose frame size is smaller than the smallest Mountain bike
select brand from bikes where framesize < (select min(framesize) from bikes where type='mountain');

-- Q11: List all Road bikes that are more expensive than the average Road bike
select * from bikes;
select brand from bikes where type='road'and  price > (select avg(price) from bikes where type='road');

-- Q12: Show all bikes with stock equal to the lowest stock among Touring bikes
select * from bikes;
select brand from bikes where  stock = (select min(stock) from bikes where type='touring');

-- Q13: Find the bike(s) that cost more than all Road bikes
select * from bikes where price > all (select max(price) from bikes where type='road');

-- Q14: Find all bike types that have an average price above $1000
select distinct type from bikes where type  in  (select type  from bikes group by type having avg(price) > 1000.00);

-- Q15: List all bikes from brands that have at least one Touring bike
select * from bikes;
select *
from bikes
where brand in  (select brand from bikes where  type='touring');
