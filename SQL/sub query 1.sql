...
-- Retrieve the brand and model of the car with the highest mileage. 
select brand,model from cars
where mileage=(select max(mileage) from cars);

-- List cars with prices higher than the average price of cars with the fuel type = 'Gasoline'.
...
select brand  as modelcount from cars
having count(model)  > (select count(brand) from cars group by brand);

-- Retrieve cars with prices closest to the average price, rounded to the nearest hundred.
select * ,avg(price) from cars
where price >=avg(price) and avg(price)=100;
