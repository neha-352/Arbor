...
-- que1 What is the total sales amount for each category?
select category,sum(sale_id) as total_sales
from sales
group by category;

-- que2 Which categories have more than 10 sales?
 select category,count(*) as total_sales
 from sales
 group by category
having count(*) > 10;
 
 -- que3 What is the maximum sale amount in each category?
select category ,max(sale_amount) as maximum_amount
from sales
group by category;

-- que4 Which categories have an average sale amount below 50?
select category,avg(sale_amount) as average_sale_amount
from sales
group by category
having avg(sale_amount) < 50;

-- que5 How many sales does each product have?
select product_name,sum(sale_id)
from sales 
group by product_name;

-- que6 Which products have more than 5 sales?
select product_name,count(*)
from sales 
group by product_name
having count(*) > 5;

-- que7 What is the average sale amount for products with more than 2 sales?
select product_name,avg(sale_amount),count(*)
from sales
group by product_name
having Count(*) >2;

-- que8 Which products have the highest sale amount above 100?
select product_name,sale_amount
from sales
where sale_amount > 100;

-- que9 Which product has the highest total sales amount?
...
-- que10 Which categories have a total sales amount above 800?
select category,sum(sale_amount)
from sales
group by category
having sum(sale_amount) > 800;

-- que11 What is the average sale amount for each month?
select month(sale_date),avg(sale_amount)
from sales
group by month(sale_date)
order by month(sale_date);

-- que12 What are the products with the highest sale amount in each category?
select category,product_name,sum(sale_amount)
from sales
group by category,product_name
having sum(sale_amount) > max(sale_amount);

-- que13 How many sales occurred for each month?
select month(sale_date),count(*) as monthly_sale
from sales
group by month(sale_date);

-- que14 In which months are there more than 5 sales?
select month(sale_date),count(*) as monthly_sale
from sales
group by month(sale_date)
having count(*) > 5;

-- que15 What is the average sale amount for each product category in months with more than 3 sales?
select category,month(sale_date),avg(sale_amount)
from sales
where month(sale_amount) >3
