...
select o.orderid,concat(c.firstname, " ",c.lastname) as CutomerName
from orders as o
join customers as c
...
select o.orderid,p.productname,p.stockquantity,p.price
from orders as o
 join products as p
on o.orderid = p.productid;

-- 4. Retrieve a list of customers who made orders and the order dates
select concat(c.firstname ," ", c.lastname) as customername,o.orderdate
from customers as c
join orders as o
...
select count(o.orderid),c.firstname,c.lastname
from orders as o
...
select productname from products where price > 100.00;

-- 10. Retrieve the orders made by customers from a specific country (e.g., 'USA').
select  distinct o.orderid,c.firstname,c.lastname,c.country
from orders as o
join customers as c
on o.customerid = c.customerid
where c.country = 'USA';
...
select o.totalamount , c.firstname ,c.lastname,o.orderid
from orders as o
join customers as c
on o.customerid = c.customerid;
...
select orderid,totalamount from orders where totalamount > 500.00;
